...
class ObjectDetectionMetric():
    ...
    def update(self,
        ...
        matching = linear_sum_assignment(-matrix_IOU)
        matching = np.asarray(matching)
        self.matched = np.transpose(matching)
        # print(self.matched)
    ...
    def get_confusion(self,
        ...
        for no_class,name in enumerate(self.names_class):
            ...
            pr_value.append([name,float("{:.2f}".format(1e2*precision)),float("{:.2f}".format(1e2*recall)),float("{:.2f}".format(1e2*IOUs_avg[no_class]))])
        content += spacing
        total_sum = self.number_groundtruth_all
        if conclude:
            print(content)
        else:
            return content, matrix_confusion, total_sum, pr_value
